...
const RESPONSE_SCHEMA: Schema = {
  ...
  properties: {
    ...
    experience: {
      type: Type.ARRAY,
      description: "A list of professional experiences/jobs parsed from the notes.",
      items: {
        type: Type.OBJECT,
        properties: {
...
export const generateProfessionalCV = async (userInfo: UserInfo, rawNotes: string): Promise<ResumeResult> => {
  ...
  const systemInstruction = `
    ...
    TASK 1: MULTI-JOB EXPERIENCE PARSING
    Analyze the 'Raw Experience Notes'.
    - If the notes describe **multiple different roles or companies** (e.g. 'Job A at Company X', 'Job B at Company Y'), you MUST generate separate entries in the 'experience' array for each role.
    - IGNORE the single 'Company Name' input field if the notes clearly indicate a history of multiple companies. Use the single input only as a fallback or for the most recent role.
    ...
    - **Dates:** Infer dates from notes if possible, otherwise use 'Date'.
    ...
    - **Hard Skills:** 
        - **IF USER PROVIDED INPUT:** Use the user's provided 'Hard Skills' exactly (just clean/format them).
