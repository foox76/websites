...
const Header: React.FC = () => {
    ...
    const renderMobileLink = (link: typeof navLinks[0], index: number) => (
        <Link
            ...
            to={link.href}
            onClick={handleLinkClick}
            className="text-3xl font-serif text-white animate-menu-link-slide hover:text-brand-teal transition-colors duration-300 tappable"
    ...
    return (
        <>
            <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isDarkBg ? 'bg-brand-dark shadow-lg' : 'bg-brand-dark/80 backdrop-blur-sm'}`}>
                {/* Compact Announcement Bar with Integrated Timer */}
                <div className="bg-brand-gold text-white py-1.5 px-4 relative z-50">
                    <div className="container mx-auto flex flex-col sm:flex-row items-center justify-between gap-1.5 sm:gap-2">
                        {/* Announcement Text */}
                        <Link to="/offers" className="hover:underline flex items-center gap-2 text-xs sm:text-sm font-medium">
            ...
            {isOpen && (
                <div className="lg:hidden fixed inset-0 bg-brand-dark/95 backdrop-blur-sm z-[49] animate-menu-fade-in pt-20">
