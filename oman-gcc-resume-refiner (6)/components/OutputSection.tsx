  ...
  return (
    <div className="w-full h-full flex flex-col items-center">
      ...
      <div className="flex-1 w-full overflow-y-auto p-8 flex justify-center bg-slate-100">
        ...
        <div 
          id="resume-preview" 
          className={`bg-white shadow-2xl transition-all duration-500 w-[210mm] min-h-[297mm] p-[15mm] relative flex flex-col ${isLoading ? 'blur-[1px]' : ''}`}
        >
          ...
          {/* Header */}
          <header className="border-b-2 border-slate-800 pb-4 mb-5 text-center print-header-border">
            <h1 className="print-name text-3xl font-bold text-slate-900 uppercase tracking-tight mb-1">
              {displayFullName}
            </h1>
            <h2 className="print-job-title text-lg font-medium text-emerald-800 mb-2">
              {displayJobTitle}
            </h2>
          ...
          <div className="space-y-5 flex-1">
            ...
            {/* Summary */}
            <section>
              <h3 className="print-section-header text-xs font-bold text-slate-900 uppercase tracking-wider border-b border-slate-300 pb-1 mb-2">
                Professional Summary
              </h3>
              <p className={`print-body-text text-justify leading-snug text-xs text-slate-700 ${!result && 'text-slate-400 italic'}`}>
                {displaySummary}
              </p>
            </section>
                ...
                {experienceData.map((job, idx) => (
                  <div key={idx}>
                    <div className="flex justify-between items-baseline mb-1">
                       <h4 className="print-job-row font-bold text-slate-800 text-sm">
                       ...
                       <span className="print-date text-slate-500 text-[10px] italic whitespace-nowrap">{job.startDate} - {job.endDate}</span>
                    </div>
                    
                    <ul className="list-disc list-outside ml-3 space-y-1">
                      {job.bulletPoints.map((point, i) => (
                        <li key={i} className={`print-body-text pl-1 leading-snug text-xs text-slate-700 ${!result && 'text-slate-400 italic'}`}>
                          {point}
                        </li>
                      ))}
                    </ul>
             ...
             <div className="grid grid-cols-2 gap-6">
               ...
               <div className="space-y-4">
                 ...
                 <section>
                   ...
                   <div className={`print-body-text text-slate-700 text-xs ${(!displayLanguages && isEmpty) && 'text-slate-400 italic'}`}>
                  ...
                  <div className="flex flex-wrap gap-1.5 text-[10px] text-slate-700">
                    {refinedHardSkills.length > 0 ? (
                      refinedHardSkills.map((skill, index) => (
                         <span key={index} className="print-skill-tag bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">{skill}</span>
                      ))
                    ) : (
